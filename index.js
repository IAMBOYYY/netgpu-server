...
async function processWithNVIDIA(job) {
  const prompt = `You are a GPU compute processor. Process this ${job.type} task: "${job.operation || job.type}". File: ${job.fileName || 'input'}. Provide a brief, helpful result summary in 2-3 sentences.`;
  ...
  const response = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model:       'meta/llama-3.1-8b-instruct',   // ✅ Fixed model — available on free tier
      max_tokens:  256,
      temperature: 0.7,
      messages: [
...
async function processWithCloudflare(job) {
  // Using text generation model (works for all image operations in demo)
  const model  = '@cf/meta/llama-3.1-8b-instruct';
  const prompt = `Process this image task: ${job.operation || 'transform'} on file: ${job.fileName || 'image'}. Describe what the result would look like in 2-3 sentences.`;
  ...
  const data = await response.json();
  return data.result?.response || `Image processed via Cloudflare. Operation: ${job.operation || 'transform'} complete.`;
}
...
function tryAssignQueuedJobs() {
  for (let i = jobQueue.length - 1; i >= 0; i--) {
    ...
    if (donor) {
      ...
      // Timeout: re-queue if donor doesn't finish in 2 min
      setTimeout(() => {
